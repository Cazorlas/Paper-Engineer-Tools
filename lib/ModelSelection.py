...
class ModelSelection:
    ...
    @staticmethod
    def PickElementByFilter(uidoc, filter):
        '''
        Select A Element By ISelectionFilter.
        '''
        object_type = ObjectType.Element
        pick_reference = uidoc.Selection.PickObject(object_type,filter, "Select Model Element")
        pick_element = uidoc.Document.GetElement(pick_reference)
        return pick_element
    
    @staticmethod
    def PickElementsByFilter(uidoc, filter):
        '''
        Select Elements By ISelectionFilter.
        '''
        object_type = ObjectType.Element
        pick_references = uidoc.Selection.PickObjects(object_type,filter, "Select Model Element")
        pick_elements = [uidoc.Document.GetElement(r) for r in pick_references]
        return pick_elements
