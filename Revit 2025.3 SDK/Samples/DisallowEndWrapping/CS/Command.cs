   ...
   public class Command : IExternalCommand
   {
         ...
         , ref string message, ElementSet elements)
      {
         try
         {
            ...
            // just support selecting only one wall
            Element selectedElem = null;
            foreach (ElementId elementId in commandData.Application.ActiveUIDocument.Selection.GetElementIds())
            {
               selectedElem = document.GetElement(elementId);
               break;
            }

            using (WallInformationForm infoForm = new WallInformationForm(selectedElem as Wall))
            {
               infoForm.ShowDialog();
            }
