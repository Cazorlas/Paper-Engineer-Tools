...
def GetAllViewInSheet():
    # Danh sách chứa các view được đặt trên sheet
    views_on_sheet = []

    # Chỉ thực hiện nếu active view là một DrawingSheet
    if activeView.ViewType == ViewType.DrawingSheet:
        # Thu thập tất cả các viewport trên sheet hiện tại
        viewports = FilteredElementCollector(doc, activeView.Id).OfClass(Viewport).ToElements()
        for vp in viewports:
            # Lấy view được gán cho viewport
...
try:
    ...
    grid_activeview = Flattern(grid_activeview)

    # print(grid_activeview)
    "----------------------------RUN FORMS----------------------------"
    f = MainForm()
    f.ShowDialog()
    "----------------------------ACTION----------------------------"
